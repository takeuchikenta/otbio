...
def main():
    ...
    feature_name = 'wl'  # 使用する特徴量名 ('wl', 'ptp', 'rms', 'zc' など)
    ...
    for subject in subjects:
        ...
        for electrode_place in ["original2","downleft5mm", "downleft10mm", "clockwise"]:
            ...
            for target_shot_index in range(5):
                X_train, y_train = create_dataset(emg_list_train, y_list_train, fs=2000, window_ms=200, step_ms=50, mode='query', shot_trial_idx=target_shot_index, feature_name=feature_name)
                X_test, y_test = create_dataset(emg_list_train, y_list_train, fs=2000, window_ms=200, step_ms=50, mode='support', shot_trial_idx=target_shot_index, feature_name=feature_name )
                # Tensor変換
                ...
                with torch.no_grad():
                    # クエリセットで評価
                    query_embeddings = model(X_test_t)
                    dists = euclidean_dist(query_embeddings, prototypes)
                    y_pred = torch.argmin(dists, dim=1)
                    score = (y_pred == y_test_t).float().mean().item()

                    all_results_data.append({
                        "Subject": subject,
                        "Method": "PN(Intra-session)",
    ...
    # ### 修正点4: CSV出力 ###
    print("\n" + "="*40)
    print("Saving results to 'pn_intrasession_results.csv'...")
    df_results = pd.DataFrame(all_results_data)
    df_results.to_csv("pn_intrasession_results.csv", index=False)
