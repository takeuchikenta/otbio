...
def main(feature_name_for_filename = "ptp", normalize = False):
    ...
    icc2_list_all = []
    r_list_all = []
    for electrode_place in ["original", "upright", "downright", "downleft", "upleft", "clockwise", "anticlockwise"]:
        theta_list = []
        dx_list = []
        dy_list = []
        scale_list = []
        ...
        for gesture in range(1,8):
            for trial1 in range(1,6):
                for trial2 in range(trial1,6):
                    if trial1 != trial2:
                        file_name1 = file_name_output(subject='garu', hand='right', electrode_place=electrode_place, gesture=gesture, trial=trial1)
