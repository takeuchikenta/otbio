...
# === main ===
def main(subject="takeuchi", registration='affine', icc_r = True):
    print("セッション間")
                        ...
                        if icc_r:
                            ...
                            for i, session in enumerate(session_list):
                                ...
                                for k, emg_data in enumerate(emg_list[(gesture-1)*5 : (gesture-1)*5 +5]):
                                    ...
                                    if i==0 and k==0:
                                        ...
                                        feature_df_icc['gesture'] = gesture
                                        feature_df_icc['trial'] = trl
                                        ...
                                        tmp_df_icc['gesture'] = gesture
                                        tmp_df_icc['trial'] = trl
                            ...
                            for gesture in feature_df_icc_long['gesture'].unique():
                                for channel in feature_df_icc_long['channel'].unique():
                                    ...
                                    icc2_ci95_lower = icc[icc['Type']=='ICC2']['CI95%'].values[0][0]
                                    icc2_ci95_upper = icc[icc['Type']=='ICC2']['CI95%'].values[0][1]
                                    icc2k_value = icc[icc['Type']=='ICC2k']['ICC'].values[0]
                                    icc2k_ci95_lower = icc[icc['Type']=='ICC2k']['CI95%'].values[0][0]
                                    icc2k_ci95_upper = icc[icc['Type']=='ICC2k']['CI95%'].values[0][1]
                                    if not np.isnan(icc2_value):
                                        icc2_list_each.append(icc2_value)
                                        icc2_results_data.append({
                                            "Subject": subject,
                                            "Position": electrode_place,
                                            "channel": channel,
                                            "icc": icc2_value,
                                            "ci95_lower": icc2_ci95_lower,
                                            "ci95_upper": icc2_ci95_upper
                                        })
                                    if not np.isnan(icc2k_value):
                                        icc2k_list_each.append(icc2k_value)
                                        icc2k_results_data.append({
                                            "Subject": subject,
                                            "Position": electrode_place,
                                            "channel": channel,
                                            "icc": icc2k_value,
                                            "ci95_lower": icc2k_ci95_lower,
                                            "ci95_upper": icc2k_ci95_upper
                                        })
    ...
    # ### CSV出力 ###
    print("\n" + "="*40)
    print("Saving ICC2 results to 'icc2_results.csv'...")
    df_icc2 = pd.DataFrame(icc2_results_data)
    df_icc2.to_csv("icc2_results" + subject + ".csv", index=False)
    print("\n" + "="*40)
    print("Saving ICC2k results to 'icc2k_results.csv'...")
    df_icc2k = pd.DataFrame(icc2k_results_data)
    df_icc2k.to_csv("icc2k_results" + subject + ".csv", index=False)
    print("Done.")