...
def main(feature_name='wl'):
    ...
    # 全被験者の結果を統合するコンテナ
    # global_results[Method][Position] = [acc_subject1_fold1, ..., acc_subject4_fold5]
    global_results = {
        "Method1 (Adaptive)": {pos: [] for pos in test_positions},
        "Method2 (Source)": {pos: [] for pos in test_positions}
    }
    ...
    for subject in subjects:
        ...
        for epoch in range(n_epochs):
            ...
            if epoch == n_epochs - 1:
                source_prototypes = prototypes.detach()
        ...
        for electrode_place in test_positions:
            ...
            # 5-fold evaluation per position
            acc_buffer_m1 = []
            acc_buffer_m2 = []
            ...
            for target_shot_index in range(5):
                ...
                with torch.no_grad():
                    ...
                    # --- Method 1 (prototypical.py): Adaptation ---
                    ...
                    # --- Method 2 (prototypical2.py): Source Only ---
                    ...
                    global_results["Method2 (Source)"][electrode_place].append(score_m2)
                    
                    acc_buffer_m1.append(score_m1)
                    acc_buffer_m2.append(score_m2)

            # 被験者ごとの進捗表示 (任意)
            print(f"    {electrode_place}: M1={np.mean(acc_buffer_m1):.3f}, M2={np.mean(acc_buffer_m2):.3f}")
    ...
    print("\n>>> Comparison: Method 1 (Adaptation) vs Method 2 (Source Only) per Position")
    
    for electrode_place in test_positions:
        acc_list_m1 = global_results["Method1 (Adaptive)"][electrode_place]
        acc_list_m2 = global_results["Method2 (Source)"][electrode_place]
    ...
    print(f'Method2 (SourceOnly) accracy_all: {np.mean(all_acc_m2_mean):.4f}')
    print("-"*60)
