...
def plot_results(df):
    ...
    # 【変更点1】Positionを 'original2' だけに絞り込み
    target_position = 'original2'
    df = df[df['Position'] == target_position]

    # 箱ひげ図
    ax = sns.boxplot(x='Method', y='Accuracy', hue='Method', data=df, showfliers=False)
    # データ点を重ねて表示（ストリッププロット）
    sns.stripplot(x='Method', y='Accuracy', hue='Method', data=df, 
