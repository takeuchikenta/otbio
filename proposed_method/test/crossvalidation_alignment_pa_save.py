...
def main():
    ...
    for subject in subjects:
        ...
        for electrode_place in ["original2", "downleft5mm", "downleft10mm", "clockwise"]:
            ...
            # if True:
                # gesture = 1  # 今回はジェスチャー1に対して位置合わせを実施
    ...
    # ### 修正点4: CSV出力 ###
    print("\n" + "="*40)
    print("Saving results to 'align_pa_results_allgestures.csv'...")
    df_results = pd.DataFrame(all_results_data)
    df_results.to_csv("align_pa_results_allgestures.csv", index=False)
    print("Done.")